...
@Component({
  selector: 'app-ecommerce-detail-sheet',
      ...
      <div class="sheet" (click)="$event.stopPropagation()">
        ...
        <div class="body">
          ...
          <div class="receipt">
            ...
            @for (item of booking().items; track item.serviceName + item.variantSize + item.variantColor) {
              <div class="receipt-item">
                <div>
                  <div class="receipt-item-name">{{ item.serviceName }}</div>
                  @if (item.variantColor || item.variantSize) {
                    <div class="receipt-item-variant">{{ variantLabel(item.variantColor, item.variantSize) }}</div>
                  }
          ...
          @if (cancelError()) { <p class="form-error">{{ cancelError() }}</p> }
  ...
  styles: `
    ...
    .handle-row { padding: 10px 0 6px; text-align: center; flex-shrink: 0; }
    .handle { width: 36px; height: 4px; background: oklch(90% 0.010 28); border-radius: 2px; display: inline-block; }
    ...
    .order-label { font-size: 12px; font-weight: 800; color: oklch(38% 0.11 145); letter-spacing: 0.5px; }
    ...
    .actions { display: flex; flex-direction: column; gap: 8px; margin: 0 0 20px; }
    ...
    .btn-danger { border-color: oklch(88% 0.04 22); color: oklch(38% 0.13 22); }
    .btn-ghost:disabled { opacity: 0.5; cursor: not-allowed; }

    .timeline { margin-bottom: 20px; }
    .tl-item { display: flex; gap: 14px; align-items: stretch; }
    .tl-dot-col { display: flex; flex-direction: column; align-items: center; flex-shrink: 0; width: 14px; }
    ...
    .tl-label { font-weight: 800; font-size: 14px; color: oklch(65% 0.008 30); margin-bottom: 2px; }
    .tl-label-done { color: #1a1a1a; }
    .tl-label-current { color: oklch(38% 0.11 145); }
    .tl-label-cancelled { color: oklch(38% 0.13 22); }
    .tl-desc { font-size: 12px; color: oklch(50% 0.01 30); line-height: 1.5; }
    ...
    .qr-tap { background: transparent; border: none; cursor: pointer; display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 0; }
    .qr-thumb-wrap { width: 96px; height: 96px; background: #fff; border: 1px solid oklch(90% 0.010 28); border-radius: 10px; display: flex; align-items: center; justify-content: center; overflow: hidden; }
    .qr-thumb-img { width: 100%; height: 100%; object-fit: contain; }
    .qr-scan-label { font-size: 12px; font-weight: 700; color: oklch(38% 0.11 145); }
    ...
    .i-paid-btn:disabled { opacity: 0.6; cursor: not-allowed; }
    ...
    .tracking-text { font-size: 13px; font-weight: 600; color: oklch(38% 0.10 72); text-align: center; }
    .track-btn {
      display: block;
      width: 100%;
    ...
    .receipt-title { font-weight: 800; font-size: 15px; color: #1a1a1a; }
    .receipt-ts { font-size: 12px; color: oklch(50% 0.01 30); }
    ...
    .receipt-row span { font-size: 12px; color: oklch(65% 0.008 30); flex-shrink: 0; }
    .receipt-row strong { font-size: 13px; color: #1a1a1a; font-weight: 700; text-align: right; }

    .receipt-items-label { font-size: 12px; color: oklch(65% 0.008 30); margin-bottom: 8px; }
    .receipt-item { display: flex; justify-content: space-between; gap: 12px; align-items: flex-start; margin-bottom: 8px; }
    .receipt-item-name { font-weight: 700; font-size: 14px; color: #1a1a1a; }
    .receipt-item-variant { font-size: 12px; color: oklch(50% 0.01 30); }
    .receipt-item-sub { font-size: 12px; color: oklch(50% 0.01 30); }
    .receipt-item-total { font-weight: 700; font-size: 14px; color: #1a1a1a; white-space: nowrap; }
    ...
    .receipt-total span { font-weight: 800; font-size: 15px; color: #1a1a1a; }
    .receipt-total strong { font-weight: 800; font-size: 15px; color: oklch(38% 0.11 145); }

    .form-error { margin-top: 10px; font-size: 13px; color: oklch(38% 0.13 22); }
    ...
    .qr-fullscreen-img { width: min(86vw, 86vh); height: min(86vw, 86vh); object-fit: contain; background: #fff; padding: 16px; border-radius: 16px; }
    .qr-fullscreen-hint { font-size: 13px; color: rgba(255, 255, 255, 0.45); }
  `
})
export class EcommerceDetailSheetComponent {
  ...
  readonly currencyCodeFallback = input<string>('USD');
  ...
  protected variantLabel(color: string | null, size: string | null): string {
